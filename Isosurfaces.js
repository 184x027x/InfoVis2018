function Isosurfaces( volume, isovalue )
{
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
                    ...
                    //var C0 = new THREE.Color(v01.x,v01.y,v01.z);
                    //var C1 = new THREE.Color(v23.x,v23.y,v23.z);
                    //var C2 = new THREE.Color(v45.x,v45.y,v45.z);
                }
            }
            cell_index++;
        }
    ...
    {
        var idv0 = v0.x*1+v0.y*volume.resolution.x+v0.z*volume.resolution.x*volume.resolution.y;
        var idv1 = v1.x*1+v1.y*volume.resolution.x+v1.z*volume.resolution.x*volume.resolution.y;
        var s0 = volume.values[idv0][0];
        var s1 = volume.values[idv1][0];
        return new THREE.Vector3(v0.x+(v1.x-v0.x)*(s0-s)/(s1-s0),v0.y+(v1.y-v0.y)*(s0-s)/(s1-s0),v0.z+(v1.z-v0.z)*(s0-s)/(s1-s0));
    }
}
